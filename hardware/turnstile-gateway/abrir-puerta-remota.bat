@echo off
title Sport Gym - Abrir puerta remota
setlocal
cd /d "%~dp0"
...
echo ========================================
echo  Sport Gym - Apertura remota torniquete
echo ========================================
echo Destino: %TARGET_IP%:%TARGET_PORT%
echo.

powershell -NoProfile -Command ^
  "try { Invoke-RestMethod -Method POST -Uri 'http://%TARGET_IP%:%TARGET_PORT%/gate/sync' -Headers @{ 'X-Device-Key'='%DEVICE_KEY%' } -ContentType 'application/json' -Body '{\"action\":\"open\"}' -TimeoutSec 8 | Out-Null; Write-Host 'OK: puerta abierta.' -ForegroundColor Green; exit 0 } catch { Write-Host ('ERROR: ' + $_.Exception.Message) -ForegroundColor Red; exit 1 }"
